...
// Looking for options in --compac-timing and --compac-options:
//
...
static void set_timing_mode(int this_option_offset, struct cgpu_info *gekko)
{
	...
	if (opt_compac_timing == NULL)
		buf[0] = '\0';
	else {
		ptr = opt_compac_timing;
...
static void get_options(int this_option_offset, struct cgpu_info *gekko, int *baud, int *work_division, int *fpga_count)
{
	...
	if (opt_compac_options == NULL)
		buf[0] = '\0';
	else {
		ptr = opt_compac_options;
		for (i = 0; i < this_option_offset; i++) {
			comma = strchr(ptr, ',');
			if (comma == NULL)
				break;
...
static struct api_data *gekko_api_stats(struct cgpu_info *cgpu)
{
	...
	root = api_add_int(root, "Frequency", &(info->opt_compac_freq), false);
